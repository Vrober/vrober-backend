...
export const getAllHomeSections = asyncHandler(async (req, res) => {
  ...
  const populatedSections = await Promise.all(
    sections.map(async (section) => {
      ...
      if (section.selectionMode === 'manual') {
        ...
        if (section.autoFilter?.field && section.autoFilter?.value !== undefined) {
  ...
  res.status(200).json(
    new ApiResponse(200, populatedSections, 'Home sections fetched successfully')
  );
});
...
export const getServicesForFilter = asyncHandler(async (req, res) => {
  ...
  const services = await Service.find(filter)
    .select('serviceName imageUrl price category isPopular isPremium isMostBooked bookingCount')
