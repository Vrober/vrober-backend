...
export async function createCallback(req, res) {
  try {
    ...
    const doc = await CallbackRequest.create({ name, phone, preferredTime, note });
    res.status(201).json({ message: 'Callback request submitted', request: doc });
  } catch (err) {
    res.status(500).json({ message: 'Failed to submit callback request', error: err.message });
  }
}
...
export async function listCallbacks(req, res) {
  try {
    ...
    res.status(200).json({ requests: items, totalPages: Math.ceil(total / limit), total, currentPage: page });
  } catch (err) {
    res.status(500).json({ message: 'Failed to list callback requests', error: err.message });
  }
}
...
export async function updateCallbackStatus(req, res) {
  try {
    ...
    const updated = await CallbackRequest.findByIdAndUpdate(id, { status }, { new: true });
